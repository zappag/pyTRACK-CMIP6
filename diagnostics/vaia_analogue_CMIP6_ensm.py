    ...
    for track_id, track_data in track_ref.items():
        ...
        if alltracks:
            ...
            for track_id, track_data in alltracks.items():
                ...
                color = np.random.choice(gray_shades)
...
def plot_allensm_tracks(track_ref, alltracks, plotdir, lat1=None, lat2=None, lon1=None, lon2=None, latgen=None, longen=None, radgen=None, latpas=None, lonpas=None, radpas=None):
    for track_id, track_data in track_ref.items():
        header_ref = track_data["header"]
        data_ref = track_data["data"]
        ...
        # Plot the reference trajectory as a line segment
        ax.plot(lon_values_ref, lat_values_ref, color="black",
                linewidth=1.5,
                transform=projection,
                label="Vaia")
        ...
        ax.set_title("All Ensemble Tracks (Var: {} - Seas: {} - Model: {})".format(var, seas, model))
        
        # Add genesis circle and passage circles as polygons
        if latgen and longen and radgen:
            print("adding genesis circle at lat: {}, lon: {}, rad: {}".format(latgen, longen, radgen))
            circle = plt.Circle((longen, latgen), radgen+1, color='red', fill=False, linewidth=1.5, transform=ccrs.PlateCarree(), label="Genesis")
            ax.add_patch(circle)
        if latpas and lonpas and radpas:
            print("adding passage circle at lat: {}, lon: {}, rad: {}".format(latpas, lonpas, radpas))
            circle = plt.Circle((lonpas, latpas), radpas+1, color='blue', fill=False, linewidth=1.5, transform=ccrs.PlateCarree(), label="Pass")
            ax.add_patch(circle)
        ...
        # Save plot
        plt.savefig(plotdir + f"allensm_tracks_vaiaanalogue_{var}_{seas}_{model}.png")
        print("saved plot named: ", plotdir + f"allensm_tracks_vaiaanalogue_{var}_{seas}_{model}.png")
...
def plot_track_density(track_ref, alltracks, plotdir, lat1=None, lat2=None, lon1=None, lon2=None):
    ...
    for track_id, track_data in track_ref.items():
        ...
        # Save plot
        plt.savefig(plotdir + f"vaia_trackdensity_{var}_{seas}_{model}_allensm.png")
        print("saved plot named: ", plotdir + f"vaia_trackdensity_{var}_{seas}_{model}_allensm.png")
        plt.close()
...
def read_all_ensemble_tracks(base_dir, model, seas, var):
        ensemble_tracks = {}
        ensemble_dirs = glob.glob(os.path.join(base_dir, model, "historical", seas, "*", var, "vaia_analogue"))
        
        for ensm_dir in ensemble_dirs:
            ensm = os.path.basename(os.path.dirname(os.path.dirname(ensm_dir)))
            print("ensm_dir is: ", ensm_dir)
            concatenated_tracks_filename, latgen, longen, radgen, latpas, lonpas, radpas = read_filtered_tracks(ensm_dir)
            all_tracks = read_ERA5_tracks(ensm_dir, concatenated_tracks_filename)
...
all_tracks = {}  # Initialize all_tracks to avoid undefined variable error

# Read all ensemble tracks
ensemble_tracks = read_all_ensemble_tracks("/home/ghinassi/work/track_output/CMIP6", model, seas, var)

# Plotting for each ensemble member
for ensm, data in ensemble_tracks.items():
    ...
    # Plot all ensemble tracks together
    combined_tracks = {}
    for ensm, data in ensemble_tracks.items():
...
if track_density:
    ...
    plot_allensm_tracks(vaia_track, combined_tracks, plotdir, lat1=30, lat2=65, lon1=-10, lon2=30, latgen=latgen, longen=longen, radgen=radgen, latpas=latpas, lonpas=lonpas, radpas=radpas)