...
ERA5_prec_dir = "/mnt/naszappa/ghinassi/ERA5/total_precipitation/total_precipitation/6hT00/SON/"
precipitation_filename = "ERA5_total_precipitation_6hT00_2018_SON.nc"
...
def plot_track_and_prec(track_ref, ERA5_mslp_dir, mslp_filename, ERA5_prec_dir, prec_filename, plotdir, label_name="Storm", lat1=None, lat2=None, lon1=None, lon2=None):
    ...
    for track_id, track_data in track_ref.items():
        ...
        cont2 = ax.contourf(lon_values_prec, lat_values_prec, prec_values*1000, levels=np.arange(0, 270.1, 10), cmap=custom_cmap, transform=projection)
        ...
        ax.legend()
        ax.set_title(f"Cumulated Precipitation")
        # Add a single colorbar for the entire figure
        cbar_ax = fig.add_axes([0.25, 0.02, 0.5, 0.02])  
        cbar=fig.colorbar(cont2, cax=cbar_ax, orientation='horizontal', extend='max')
        # font size for label
        ...
        # Save plot
        plt.savefig(os.path.join(plotdir, f"mslp_total_prec_6h_{track_id}.png"), bbox_inches="tight", dpi=600)
        print(f"Saved plot with MSLP and precipitation in: {plotdir}/mslp_total_prec_6h_{track_id}.png")

        plt.close()
        

vaia_track = read_ERA5_tracks(ERA5_track_dir_vaia, ERA5_track_vaia_filename)
plot_track_and_prec(vaia_track, ERA5_mslp_dir, mslp_filename, ERA5_prec_dir, precipitation_filename, plotdir, label_name="Vaia", lat1=30, lat2=55, lon1=1, lon2=25)